...
public interface SamrukRepo extends JpaRepository<Samruk, Long> {
    ...
    @Query(value = "select * from imp_kfm_db.samruk_kazyna where extract(year from contract_date) = ?2 and supplier = ?1 limit 10 offset ?3", nativeQuery = true)
    List<Samruk> getBySupplierAndYear(String bin, Integer year, Integer page);

    @Query(value = "select * from imp_kfm_db.samruk_kazyna where extract(year from contract_date) = ?2 and customer = ?1 limit 10 offset ?3", nativeQuery = true)
    List<Samruk> getByCustomerAndYear(String bin, Integer year, Integer page);

    @Query(value = "select * from imp_kfm_db.samruk_kazyna where contract_date is NULL  and supplier = ?1 limit 10 offset ?2", nativeQuery = true)
    List<Samruk> getBySupplierAndNullYear(String bin, Integer page);

    @Query(value = "select * from imp_kfm_db.samruk_kazyna where contract_date is NULL  and customer = ?1 limit 10 offset ?2", nativeQuery = true)
